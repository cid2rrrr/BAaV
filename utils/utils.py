...
def _eval_pr(y_pred, y, num, threshold, cuda_flag=False):
    if cuda_flag:
...
class Evaluator_iiou(object):
    ...
    def cal_CIOU(self, infer, gtmap, thres=0.01):
        ...
        #!#!#   
        ciou = np.sum(infer_map*gtmap) / (np.sum(gtmap) + np.sum(infer_map * (gtmap==0)))
        intersection = np.sum(infer_map * gtmap)
        FP = np.sum(infer_map * (gtmap==0))
        FN = np.sum(gtmap * (infer_map==0))
        
        beta_squared = 0.3
        precision = intersection / (intersection + FP) if (intersection + FP) > 0 else 0
        recall = intersection / (intersection + FN) if (intersection + FN) > 0 else 0
        
        f_beta = (1 + beta_squared) * (precision * recall) / (beta_squared * precision + recall) \
            if (beta_squared * precision + recall) > 0 else 0
        self.fscore.append(f_beta)
        # ciou = intersection / union if union > 0 else 0
        #!#!#
        if ciou != ciou:
            ciou = 0.0
        self.ciou.append(ciou)
    ...
    def mask_iou(self, pred, target, threshold, eps=1e-7, size_average=True, mode='adaptive'):
        # print(pred)
        # print(target)
        ...
        no_obj_flag = (target.sum() == 0)

        # temp_pred = torch.sigmoid(pred)
        temp_pred = pred
        pred = (temp_pred > threshold).int()
        if not no_obj_flag:
            inter = (pred * target).sum()
            union = torch.max(pred, target).sum()
        else:
            inter = num_pixels - pred.sum()
            union = num_pixels
            # print('no_obj')
            # print(inter)

        iou = torch.sum(inter / (union+eps)) / (N+eps)
        
        if mode == 'adaptive':
            self.ciou_adap.append(iou)
        else:
            self.ciou.append(iou)
        return iou
    
    

    def Eval_Fmeasure(self, pred, gt, threshold, pr_num=255, mode='adaptive'):
        r"""
            param:
                pred: size [N x H x W]
                gt: size [N x H x W]
        ...
        for img_id in range(N):
            ...
            if torch.mean(gt[img_id]) == 0.0:
                continue
            prec, recall = _eval_pr(pred[img_id], gt[img_id], pr_num, threshold)
        ...
        if torch.mean(gt[img_id]) == 0.0:
            return 0
        if mode == 'adaptive':
            self.fscore_adap.append(score.max().item())
        else:
            self.fscore.append(score.max().item())
    ...
    def cal_CIOU_adap(self, infer, gtmap, thres=0.01):
        ...
        f_beta = (1 + beta_squared) * (precision * recall) / (beta_squared * precision + recall) \
            if (beta_squared * precision + recall) > 0 else 0
        self.fscore_adap.append(f_beta)
        #!#!#
